...
| `PDF_PARSE_HEAP_BUDGET_RATIO` | How much of the process heap (between 0 and 1) a single PDF parse may grow the heap by before it is abandoned during indexing. Stops a pathological PDF from exhausting the indexing worker's memory before the timeout fires. Measured against each parse's own starting point, not the absolute heap level. Only applies when `TIKA_URL` is not set. | `0.25`                  |
